...
class SocketIOConnection {
	private static final String TAG = "android-websockets:SocketIOConnection";
	...
	public void emitRaw(int type, SocketIOClient client, String message,
		...
		webSocketClient.send(String.format("%d:%s:%s:%s", type, ack,
		...
		httpClient.executeString(request, new AsyncHttpClient.StringCallback() {
			...
			public void onCompleted(final Exception e, String result) {
				...
				try {
					...
					// hardcode wss - live/ws - sandbox
					String scheme = uri.getScheme().equals("https") ? "wss"
							: "ws";
					final String sessionUrl = scheme + "://" + uri.getHost()
							+ ":" + uri.getPort() + uri.getPath()
							+ "websocket/" + session;

					if (WebSocketClient.isDebugMode()) {
						Log.d(TAG + ".onCompleted mySessionUrl", sessionUrl);
					}

					SocketIOConnection.this.webSocketClient = new WebSocketClient(
							URI.create(sessionUrl), new WebSocketClient.Listener() {
								...
								public void onMessage(String message) {
									try {
										...
										case 6:
											// ACK
											final String[] ackParts = parts[3]
													.split("\\+", 2);
											Acknowledge ack = acknowledges
													.remove(ackParts[0]);
											if (ack == null)
												return;
								...
								public void onDisconnect(int code, String reason) {
									if (/* reason.equals("EOF") && */!reason
											.equals("SSL")
											&& disconnected
											&& doReconnect) {
										reconnect();
									} else {
										reportDisconnect(new IOException(
												String.format(
														"Disconnected code %d for reason %s",
														code, reason)));
									}
								}
		...
		select(null, new SelectCallback() {
			...
			public void onSelect(final SocketIOClient client) {
				if (client.connected) {
					...
					final DisconnectCallback closed = client
	...
	private Acknowledge acknowledge(final String messageId) {
		...
		return new Acknowledge() {
			...
			public void acknowledge(JSONArray arguments) {
				...
				webSocketClient
						.send(String.format("6:::%s%s", messageId, data));
			}
		};
	}
